...
**Project purpose:** Electric power meters can exchange data using different hardware and software APIs.
The server application collects data from the power meters and stores it in the SQL Server database.
SQL Server Agent executes jobs that calculate results from the primary data using measurement transformer factors and power grid topology history (bypass switch commutation history, measurement transformer replacement history, etc.).

**Project description:**
My application provides proper data storage corresponding to the location on the power grid where the measurements were collected. In other words, my application is the editor for creating the power network model in the database, storing data correctly to support job-based processing and reporting. My application contains a T-SQL script that can actualize any version of the database to the actual version with all necessary data conversions, a COM object which provides API and UI controls for the data structure editor, and for the different applications that work with data: viewers, report generators, etc., and a desktop editor for the tree-shaped representation of the electric energy system.

**My part in this project:** Requirements preparation, integration with existing architecture, development of the architecture, implementation, testing, deployment to the first customer, technical support, collecting and implementing additional customer requirements after they got some user experience.
...
Fig. 2 The measurement point explanation.<br>
