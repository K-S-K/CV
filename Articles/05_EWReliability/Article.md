...
Application health data collection and analysis. Availability factor measuring. A tool for the investigation of the evolution of accidents. Software for load monitoring and emergency prediction. A tool for loading testing of billing software after every sprint.

**My part in this project:**

- Theory research;
- RND (different experiments with collecting and analyzing the measures);
...
- Creating a cell commutator emulator (with traffic generator schedulers, response time, and content analyzers);
...
- Several additional measuring subsystems like SQLServer availability observation component and MongoDB Health monitoring component, because SQLServer and MongoDB don't naturally provide the data we need to observe them;
- Reliability Interval Marking on the DB subsystem, which is running on SQL Server by the SQL Server Agent, and makes ready-to-use intervals of work for every observing software instance;
- Tree-like table interval representation for the UI and the reports;
- Reliability diagram - a scalable graphical representation of the reliability intervals on a common time scale for manual visual analysis of the evolution process of emergency situations.
...
**Fig.2 The Reliability Intervals Table.** The tree-list view, for expanding the recorded working intervals history for each of the observed applications.
![Order Events lists](Images/Fig_02_RITable.png)

**Fig.3 The Reliability Intervals Diagram.** Expanded view during the Emergency situation investigation.
...
[^1]: The project is in use by a cell operator billing system to predict emergency situations and to analyze the evolution of damages and failures. Also, the project is used in a billing system life cycle to measure and compare reliability parameters, such as MTBF and Availability factor, for each release during load testing.
